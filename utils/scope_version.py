...
"""
Script to generate Scope version numbers
...
def check_existing_version(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        if "SCOPE" in file.read():
            raise ValueError("file already contains scope versioning")
...
def get_version_parts(tree, platform_tree):
    ...
    version_paths = {
        "SCOPE_MAJOR": tree / "version.txt",
        "SCOPE_MINOR": tree / "chromium_version.txt",
        "SCOPE_PATCH": tree / "revision.txt",
    }

    if platform_tree is not None:
        version_paths["SCOPE_PLATFORM"] = platform_tree / "revision.txt"

    version_parts = {}
    for name, path in version_paths.items():
        delta = 0 if name != "SCOPE_MINOR" else -CHROME_VERSION_BASE
