...
"""
Script to generate Quill version numbers
...
def check_existing_version(path):
    ...
    with open(path, "r", encoding="utf-8") as file:
        if "QUILL" in file.read():
            raise ValueError("file already contains quill versioning")
...
def get_version_parts(tree, platform_tree):
    ...
    version_paths = {
        "QUILL_MAJOR": tree / "version.txt",
        "QUILL_MINOR": tree / "chromium_version.txt",
        "QUILL_PATCH": tree / "revision.txt",
    }

    if platform_tree is not None:
        version_paths["QUILL_PLATFORM"] = platform_tree / "revision.txt"

    version_parts = {}
    for name, path in version_paths.items():
        delta = 0 if name != "QUILL_MINOR" else -CHROME_VERSION_BASE
