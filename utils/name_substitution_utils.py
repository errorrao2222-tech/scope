...
REPLACEMENT_REGEXES_STR = [
    ...
    # main replacement(s)
    (r'(\b)chrome://', r'\1quill://'),
    (r'(?:Google )?Chrom(e|ium)(?!\w)', r'Quill'),
...
def replace_text(text):
    """Replaces instances of Chrom(e | ium) with Quill in strings"""
    had_match = False
...
def replace_grit_message(msg):
    """
    Replaces instances of Chrom(e | ium) with Quill
...
def replace_xtb_translation(msg, fp_map):
    """
    Replaces instances of Chrom(e | ium) with Quill in a
    .xtb <translation>.
    """
...
def replace_grit_tree(text):
    """Replaces instances of Chrom(e | ium) with Quill, where desired"""
...
def dedup_translations_in_place(translation, seen):
    ...
    # "Chrome" variant. Since we substituted both of those with Quill,
