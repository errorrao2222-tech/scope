...
REPLACEMENT_REGEXES_STR = [
    ...
    # main replacement(s)
    (r'(\b)chrome://', r'\1scope://'),
    (r'(?:Google )?Chrom(e|ium)(?!\w)', r'Scope'),
...
def replace_text(text):
    """Replaces instances of Chrom(e | ium) with Scope in strings"""
    had_match = False
...
def replace_grit_message(msg):
    """
    Replaces instances of Chrom(e | ium) with Scope
...
def replace_xtb_translation(msg, fp_map):
    """
    Replaces instances of Chrom(e | ium) with Scope in a
    .xtb <translation>.
    """
...
def replace_grit_tree(text):
    """Replaces instances of Chrom(e | ium) with Scope, where desired"""
...
def dedup_translations_in_place(translation, seen):
    ...
    # "Chrome" variant. Since we substituted both of those with Scope,
